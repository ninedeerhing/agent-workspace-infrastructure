...
- Current operational loop: `apps/quant_assistant` PL-G explicit runner wiring design/implementation preflight mocked-only（TREE-2 data gate passed；loop208 completed real runner authorization planning bundle with worker rendezvous；2026-06-22 governance repair blocks further one-marker acceptance checklist drift；见 `harness/loop-state.json` and app §5.532）
