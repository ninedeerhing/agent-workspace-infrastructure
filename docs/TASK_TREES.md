...
- Current operational loop: `apps/quant_assistant` PL-G explicit actual-adapter dry-run authorization packet TDD mocked-only（TREE-2 data gate passed；loop227 completed actual adapter authorization preflight review with worker rendezvous；2026-06-22 governance repair blocks further one-marker acceptance checklist drift, and loop227 keeps the authorization preflight preflight-review-only/not-execution before any real runner/actual adapter dry-run/PL-H execution；见 `harness/loop-state.json` and app §5.551）
