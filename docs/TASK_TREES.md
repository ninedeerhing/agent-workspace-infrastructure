...
- Current operational loop: `apps/quant_assistant` PL-G actual adapter dry-run execution dry-run gate goal-bundle TDD mocked-only（TREE-2 data gate passed；loop228 completed explicit actual-adapter dry-run authorization packet with worker rendezvous；2026-06-22 governance repair blocks further one-marker acceptance checklist drift, and loop228 keeps the authorization packet authorization-packet-only/not-execution before any real runner/actual adapter dry-run/PL-H execution；见 `harness/loop-state.json` and app §5.552）
