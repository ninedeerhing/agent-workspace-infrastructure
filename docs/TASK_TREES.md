...
Updated: 2026-07-11T20:05:00+08:00 · SYNC-846 · controlled A/E candidate readiness UI
...
- Current operational loop: `apps/quant_assistant` 因子构造宇宙真实可用链路；loop831/SYNC-846 completed `CONTROLLED_AE_CANDIDATE_READINESS_UI_SURFACE_LOOP831` by exposing controlled A/E readiness on `/quant/factor-mining`.
...
- Next atomic action: `AE_SMALL_BATCH_SCORING_PREFLIGHT_CLOSURE_LOOP832` — let small-batch real scoring preflight consume `controlled_ae_candidate_readiness_v1` and remain blocked until A/E source-review validation closes; no DB write, scorer/backtest, Docker, or ports.
