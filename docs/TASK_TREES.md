...
- Current operational loop: `apps/quant_assistant` PL-G later executable handoff gate preflight TDD mocked-only（TREE-2 data gate passed；loop237 completed narrower implementation handoff seam with worker rendezvous；2026-06-22 governance repair blocks further one-marker acceptance checklist drift, and loop237 keeps the seam seam-only/not-execution before any real runner/adapter invocation/actual adapter dry-run/PL-H execution；见 `harness/loop-state.json` and app §5.561）
