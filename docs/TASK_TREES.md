...
- Processing rules:
  ...
  - 有变更必须写 §5 台账
- Next atomic action:
  - 日常业务继续 `apps/quant_assistant` TREE-6 / PL-G minimal real-runner dry-run eligibility reassessment TDD mocked-only；架构侧用 `.\harness\scripts\codex-self-check.ps1 -Format markdown` 防 skill/worker/goal gate 漂移
...
- Current operational loop: `apps/quant_assistant` PL-G minimal real-runner dry-run eligibility reassessment TDD mocked-only（TREE-2 data gate passed；loop220 completed authorization packet / dry-run execution preflight with worker rendezvous；2026-06-22 governance repair blocks further one-marker acceptance checklist drift, and loop220 keeps authorization packet preflight fail-closed before any real runner/actual adapter dry-run/PL-H execution；见 `harness/loop-state.json` and app §5.544）
