...
5. `docs/LOOP_ENGINEERING.md`（Loop 自治派任务）
6. `harness/loop-state.json` → **next_atomic_action**（机器态，优先于 prose）
7. `apps/quant_assistant/docs/METHODOLOGY_MEMORY.md` 最近 2 轮
8. 本文件

## 3.1 Loop 续跑（硬约束）

- 读 `.cursor/rules/raindeer-loop-autonomous.mdc`：禁止停顿式询问；完成一项即派下一项
- turn 将满：`python harness/loop_tick.py prepare`
