...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐 A/E 数据源缺口闭环。loop825/SYNC-840 已把 `source_adapter_manifest_review_from_conversion_v1` 接入候选级 Data Availability Gate：`pending_reviewer_approval` 只变成 `manifest_review_pending_reviewer_approval` partial/source-review state，不进入 calculable/ready。下一步是 `SOURCE_ADAPTER_MANIFEST_OWNER_APPROVAL_CONTRACT_LOOP826`：为 pending review items 增加 owner approval no-execution contract。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `SOURCE_ADAPTER_MANIFEST_OWNER_APPROVAL_CONTRACT_LOOP826`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-840 已把 source adapter manifest review status 连接到 Data Availability Gate 的 partial/source-review 状态。下一步要建立 owner approval no-execution contract，缺口必须继续作为下一目标推进，不是停止点。
