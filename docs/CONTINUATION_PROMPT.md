...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐 A/E 数据源缺口闭环。loop819/SYNC-834 已新增 `missing_source_onboarding_manifest_conversion_candidate_v1`，把完成材料的 source-onboarding review packet 转成 review-only source adapter manifest conversion candidate。下一步是 `MISSING_SOURCE_MANIFEST_CONVERSION_UI_SURFACE_LOOP820`：在 `/quant/factor-mining` 展示 adapter/table/PIT/coverage/owner/material refs 草案。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `MISSING_SOURCE_MANIFEST_CONVERSION_UI_SURFACE_LOOP820`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-834 已把缺源接入材料 review packet 转成 manifest conversion candidate。下一步要把 manifest 草案展示到用户可见面，缺口必须继续作为下一目标推进，不是停止点。
