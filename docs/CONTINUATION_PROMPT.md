...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `POST_ACCEPTANCE_MULTI_SOURCE_MODE_PRODUCT_DOGFOOD_LOOP879`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-892 已完成 source-mode-aware family profiles 与 recap strategy visibility。下一步用多类别产品狗粮证明 B/C/D UI 选择到 API summary/recap 的 profile 差异；缺口必须继续作为下一目标推进，不是停止点。
...
- quant_assistant business code is on TREE-6 / PL-G; current slice is `POST_ACCEPTANCE_MULTI_SOURCE_MODE_PRODUCT_DOGFOOD_LOOP879`: extend product/browser dogfood so B auto, C formula, and D idea category choices prove different UI payload sourceModeId, backend summary profile, family_summaries, and recap-visible generator strategy. No DB/Docker/scorer/backtest. The unique core mainline remains auto mining → auto backtest full flow + intent understanding state machine / intent quant subgraph.
