...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐 A/E 数据源缺口闭环。loop821/SYNC-836 已把 manifest conversion UI/API evidence 纳入 `product_self_validation_gate_v1`，batch-mining plan 顶层暴露 blocked/read-only conversion candidate。下一步是 `SOURCE_ADAPTER_MANIFEST_REVIEW_FROM_CONVERSION_LOOP822`：构建 source adapter manifest review read-model。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `SOURCE_ADAPTER_MANIFEST_REVIEW_FROM_CONVERSION_LOOP822`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-836 已把 manifest conversion surface/API evidence 纳入产品自检。下一步要把 conversion candidate 转成 source adapter manifest review read-model，缺口必须继续作为下一目标推进，不是停止点。
