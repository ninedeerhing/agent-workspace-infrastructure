...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐消费级自检和正式验收后发现的功能/产品缺口。loop895/SYNC-909 已完成 Factor Library / Jobs feedback to new MiningBrief：复盘 outcome detail 可点击进入下一轮 FactorMining，MiningBrief 写入 `generation_mix.trajectory_outcome_details`，批量复盘展示继承复盘和 success seed refs。下一步是 `POST_ACCEPTANCE_FEEDBACK_SEED_OUTCOME_VARIANT_DOGFOOD_LOOP896`。
- Current direction: 使用 CodeX permanent-worker 模式；canonical Worker 完成后必须主动把 `HOT_PATH_WORKER_REPORT_V1` 发给 Report Relay `019f59d6-f86d-75d3-9266-082079e31d71`，Relay 校验后交 Dispatcher `019f0890-af82-7ad3-a19a-d319d9aa8bb5` 直接派下一阶段，不依赖 Orchestrator 轮询。canonical threads: planner `019f0890-69e6-7270-a742-1178836608ef`, dispatcher `019f0890-af82-7ad3-a19a-d319d9aa8bb5`, executor `019eeece-c617-71c3-a80a-39a693ad3ac3`, test-engineer `019eeece-52d7-7b73-868a-7beb496ba303`, code-reviewer `019eeed1-7e14-7342-9d45-d7948aec94d2`, verifier `019eeed2-dbc0-7313-8d64-f9c6f199c68b`, report-relay `019f59d6-f86d-75d3-9266-082079e31d71`。用户在 CodeX UI 管理模型，消息不得传 model override。当前业务下一动作以 `harness/loop-state.json` 的 LOOP939R 为准。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `POST_ACCEPTANCE_FEEDBACK_SEED_OUTCOME_VARIANT_DOGFOOD_LOOP896`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-909 已完成 Factor Library / Jobs feedback to new MiningBrief。下一步补 watchlist/failure/completed-no-report 复盘种子变体 dogfood，确保它们不会被误当 success seed；缺口必须继续作为下一目标推进，不是停止点。
...
- Worker models are user-managed in the CodeX UI. Never pass a model override when creating or messaging permanent Worker threads; never create a duplicate Worker to work around model availability.
...
- quant_assistant business code is on TREE-6 / PL-G; current slice is `POST_ACCEPTANCE_FEEDBACK_SEED_OUTCOME_VARIANT_DOGFOOD_LOOP896`: add product dogfood for watchlist/failure/completed-no-report feedback seed variants so only completed report-ready success_seed expands success families. No DB/Docker/scorer/backtest unless a later explicit controlled execution loop requires it. The unique core mainline remains auto mining → auto backtest full flow + intent understanding state machine / intent quant subgraph.
