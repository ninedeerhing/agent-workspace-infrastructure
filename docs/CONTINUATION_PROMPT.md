...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `RUNTIME_DB_RUNNER_SMOKE_REENTRY_LOOP852`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-866 已通过生成页 → Jobs 确认 → 因子库复盘的产品级前端 smoke。下一步确认正确 DB/runner/API 运行态，不创建替代 Docker/DB/端口；缺口必须继续作为下一目标推进，不是停止点。
