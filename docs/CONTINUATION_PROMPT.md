...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `POST_ACCEPTANCE_IDEA_SPEC_RECAP_AND_CANDIDATE_PREVIEW_LOOP881`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-894 已完成“用户想法 -> ConstructionSpec”的自由输入路径和 confirm payload 证明。下一步让 idea_text 影响候选预览、思路摘要和复盘标记；缺口必须继续作为下一目标推进，不是停止点。
...
- quant_assistant business code is on TREE-6 / PL-G; current slice is `POST_ACCEPTANCE_IDEA_SPEC_RECAP_AND_CANDIDATE_PREVIEW_LOOP881`: make user-entered idea_text influence the visible ConstructionSpec, candidate preview, idea summary, and batch recap markers, while preserving selected A-E context and the same no-execution preview/confirmation chain. No DB/Docker/scorer/backtest. The unique core mainline remains auto mining → auto backtest full flow + intent understanding state machine / intent quant subgraph.
