...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `CONTROLLED_BACKTEST_REQUEST_REENTRY_LOOP842`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-856 已建立 backtest plan allocator provenance boundary。下一步让 controlled backtest request / execution confirmation 只消费 plan-ready drafts 并继续保持 no queue/no execution；缺口必须继续作为下一目标推进，不是停止点。
