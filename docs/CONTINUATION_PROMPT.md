...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐 A/E 数据源缺口闭环。loop815/SYNC-830 已把 `factor_universe_missing_source_onboarding_request_v1` 放到 `/quant/factor-mining` 页面，用户可见 `entity_linking_table`、`alternative_data_snapshot` 缺什么、为什么阻塞 E 类真实候选、用户需提供什么、系统接入步骤，并保持所有评分/回测/DB/Docker side effects=false。下一步是 `MISSING_SOURCE_ONBOARDING_PRODUCT_SELF_VALIDATION_LOOP816`：把该 surface 纳入产品自检硬门。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `MISSING_SOURCE_ONBOARDING_PRODUCT_SELF_VALIDATION_LOOP816`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-830 已把缺失数据源接入请求展示到 UI。下一步要纳入 product self-validation gate，缺口必须继续作为下一目标推进，不是停止点。
