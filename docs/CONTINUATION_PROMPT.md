...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `PERSISTED_PROVISIONAL_ACCEPTED_REENTRY_LOOP839`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-853 已建立 ignored quality event refs admission boundary。下一步让 persisted provisional admission/store write 只消费 provisional_ready_refs；缺口必须继续作为下一目标推进，不是停止点。
