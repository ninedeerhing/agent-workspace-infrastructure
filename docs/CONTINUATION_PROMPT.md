...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `CONTROLLED_REAL_BACKTEST_RUNNER_INJECTION_DRY_RUN_REENTRY_LOOP854`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-868 已让 API action 暴露 runner manifest，当前 runner 未接入、不会运行回测。下一步做 official runner 注入 dry-run/authorization seam，仍禁止默认 runner 和替代 runtime；缺口必须继续作为下一目标推进，不是停止点。
