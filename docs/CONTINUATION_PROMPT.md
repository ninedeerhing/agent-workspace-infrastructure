...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `POST_ACCEPTANCE_FACTOR_LIBRARY_FEEDBACK_TO_NEW_MINING_BRIEF_LOOP895`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-908 已完成 trajectory memory to generator scheduler context。下一步把 Factor Library / Jobs 复盘中的 selected outcome_details 带回新一轮 MiningBrief；缺口必须继续作为下一目标推进，不是停止点。
...
- quant_assistant business code is on TREE-6 / PL-G; current slice is `POST_ACCEPTANCE_FACTOR_LIBRARY_FEEDBACK_TO_NEW_MINING_BRIEF_LOOP895`: carry selected outcome details from Factor Library / Jobs recaps into a new FactorMining ConstructionSpec / MiningBrief so users can start the next round from recap memory without retyping. No DB/Docker/scorer/backtest unless a later explicit controlled execution loop requires it. The unique core mainline remains auto mining → auto backtest full flow + intent understanding state machine / intent quant subgraph.
