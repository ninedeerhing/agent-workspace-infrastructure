...
- Business mainline: `apps/quant_assistant` — daily_bar / daily_trade_status / adj_factor complete to 2026-06-18；loop177 已完成 `PL-G real batch demand gate API surface TDD mocked-only`
- Current direction: 使用 CodeX orchestrator-Only 模式；日常只开 orchestrator 会话；CodeX worker 通过 `create_thread`/`send_message_to_thread`；下一拍执行 `PL-G JobsPage read-only demand gate display TDD mocked-only`，只展示只读 gate summary，不接默认真实 runner
...
- Next direction: 读 `apps/quant_assistant/docs/CONTINUATION_PROMPT.md`（含 §5.500 real batch demand gate API surface 结论与 `PL-G JobsPage read-only demand gate display TDD mocked-only` 下一原子动作）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5 与 `apps/quant_assistant/docs/PROJECT_STATUS.md` §5.500 最新台账
...
- 业务隔离：loop177 仅改 read-only API surface/test 合同与真源文档；未输出 DSN/token；未读取 `.env`；未执行 trigger/runner、migration/backfill/background process/default real runner/DB-backed backtest
...
- quant_assistant business code is on TREE-6 / PL-G; loop177 exposed `real_batch_demand_gate` in the read-only mining jobs API payload from already-loaded store rows. JobsPage read-only demand gate display mocked-only remains next.
