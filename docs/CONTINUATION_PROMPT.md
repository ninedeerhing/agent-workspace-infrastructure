...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐 A/E 数据源缺口闭环。loop820/SYNC-835 已在 `/quant/factor-mining` 展示 `missing_source_onboarding_manifest_conversion_candidate_v1` 的 review-only manifest 草案。下一步是 `MISSING_SOURCE_MANIFEST_CONVERSION_PRODUCT_SELF_VALIDATION_LOOP821`：把 manifest conversion UI/API evidence 纳入产品自检门。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `MISSING_SOURCE_MANIFEST_CONVERSION_PRODUCT_SELF_VALIDATION_LOOP821`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-835 已把 manifest conversion candidate 展示到用户可见面。下一步要把该 surface 纳入产品自检，缺口必须继续作为下一目标推进，不是停止点。
