...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐 A/E 数据源缺口闭环。loop830/SYNC-845 已新增 `controlled_ae_candidate_readiness_v1`，从 Data Availability 的 `source_review_owner_approved` partial refs 形成 `source_review_validation_candidate_refs`，并在 creation plan 顶层暴露。下一步是 `CONTROLLED_AE_CANDIDATE_READINESS_UI_SURFACE_LOOP831`。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `CONTROLLED_AE_CANDIDATE_READINESS_UI_SURFACE_LOOP831`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-845 已建立 controlled A/E candidate readiness contract。下一步要把 readiness 展示给用户，缺口必须继续作为下一目标推进，不是停止点。
