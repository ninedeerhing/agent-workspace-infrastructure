...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐 A/E 数据源缺口闭环。loop826/SYNC-841 已新增 `source_adapter_manifest_owner_approval_v1` no-execution contract 并在 creation plan 顶层暴露；owner refs 齐全也只进入 source-review read-model，不标 ready。下一步是 `SOURCE_ADAPTER_MANIFEST_OWNER_APPROVAL_UI_SURFACE_LOOP827`：把 owner approval 状态展示到 `/quant/factor-mining`。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `SOURCE_ADAPTER_MANIFEST_OWNER_APPROVAL_UI_SURFACE_LOOP827`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-841 已建立 owner approval no-execution contract 并顶层暴露。下一步要做 owner approval UI surface，缺口必须继续作为下一目标推进，不是停止点。
