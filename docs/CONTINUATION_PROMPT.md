...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐消费级自检和正式验收前缺口。loop852/SYNC-867 已确认现有 qa-pg-alt DB 运行态正常，但 controlled real backtest runner 仍缺 manifest/dry-run capability，preflight fail-closed。下一步是 `CONTROLLED_REAL_BACKTEST_RUNNER_MANIFEST_REENTRY_LOOP853`。
- Current direction: 使用 CodeX orchestrator-only 模式；日常只开 orchestrator 会话；CodeX worker 优先通过跨对话 `send_message_to_thread` 续用永久 `codex_thread_id`；daily ops 只由 pinned `daily-ops` thread `019ef261-de0b-7ad0-8e9c-bb005dd38af0` 执行。canonical worker threads: test-engineer `019eeece-52d7-7b73-868a-7beb496ba303`, executor `019eeece-c617-71c3-a80a-39a693ad3ac3`, code-reviewer `019eeed1-7e14-7342-9d45-d7948aec94d2`, verifier `019eeed2-dbc0-7313-8d64-f9c6f199c68b`; `019ef130...` 是 runtime_agent_id 辅助证据，不得当 Codex thread id。下一拍必须先过 Goal/Plan Gate + Function-First Loop Gate + Skill Routing Gate + Worker Dispatch Gate + Worker Cluster/Rendezvous Gate + Skill Lifecycle Gate + Worker Capacity Gate，再执行 `CONTROLLED_REAL_BACKTEST_RUNNER_MANIFEST_REENTRY_LOOP853`；展示、UI 文案、业务原则、门禁、方法论、lifecycle 和真源同步只能作为核心功能完成后的 closing work。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `CONTROLLED_REAL_BACKTEST_RUNNER_MANIFEST_REENTRY_LOOP853`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-867 已确认 qa-pg-alt DB 运行态正常，runner 缺口为 `runner_manifest_missing,dry_run_capability_missing`。下一步补受控真实回测 runner manifest / dry-run capability read-model；缺口必须继续作为下一目标推进，不是停止点。
