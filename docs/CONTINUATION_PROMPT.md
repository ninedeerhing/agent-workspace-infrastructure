...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `BACKTEST_PLAN_ALLOCATOR_REENTRY_LOOP841`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-855 已建立 final accepted persisted written-refs-only boundary。下一步让 backtest plan allocator 只消费 final accepted refs 并继续保持 plan-only/no queue/no execution；缺口必须继续作为下一目标推进，不是停止点。
