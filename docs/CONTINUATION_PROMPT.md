...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐 A/E 数据源缺口闭环。loop824/SYNC-839 已把 `source_adapter_manifest_review_from_conversion_v1` 的 UI/API evidence 纳入 `product_self_validation_gate_v1`；缺 `has_source_adapter_manifest_review_surface` 或 `factor_universe_source_adapter_manifest_review_from_conversion` 会 fail closed。下一步是 `SOURCE_ADAPTER_MANIFEST_REVIEW_TO_DATA_AVAILABILITY_LINKAGE_LOOP825`：把 manifest review 状态接入候选级 Data Availability Gate / local source status。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `SOURCE_ADAPTER_MANIFEST_REVIEW_TO_DATA_AVAILABILITY_LINKAGE_LOOP825`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-839 已把 source adapter manifest review UI/API evidence 纳入产品自检。下一步要把 review status 连接到 Data Availability Gate 的 partial/source-review 状态，缺口必须继续作为下一目标推进，不是停止点。
