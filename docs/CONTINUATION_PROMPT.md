...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐 A/E 数据源缺口闭环。loop831/SYNC-846 已把 `controlled_ae_candidate_readiness_v1` 展示到 `/quant/factor-mining`，让用户看见 `source_review_validation_candidate_refs`、`held_candidate_refs` 和不能直接评分/回测的边界。下一步是 `AE_SMALL_BATCH_SCORING_PREFLIGHT_CLOSURE_LOOP832`。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `AE_SMALL_BATCH_SCORING_PREFLIGHT_CLOSURE_LOOP832`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-846 已展示 controlled A/E candidate readiness。下一步让小批真实评分 preflight 消费该 readiness，并在 source-review validation 未闭合前 fail-closed；缺口必须继续作为下一目标推进，不是停止点。
