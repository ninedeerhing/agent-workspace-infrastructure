...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐 A/E 数据源缺口闭环。loop814/SYNC-829 已完成 `factor_universe_missing_source_onboarding_request_v1`：为 `entity_linking_table`、`alternative_data_snapshot` 等 missing 源生成用户可读接入请求，说明缺什么、为什么阻塞 E 类真实候选、需要用户/数据提供什么、系统接入步骤，并保持所有评分/回测/DB/Docker side effects=false。下一步是 `AE_MISSING_SOURCE_ONBOARDING_UI_SURFACE_LOOP815`：把该 onboarding request 放到 `/quant/factor-mining` 用户可见页面。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `AE_MISSING_SOURCE_ONBOARDING_UI_SURFACE_LOOP815`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-829 已把缺失数据源接入请求落为后端/read-model 合同，尚未展示到 UI。缺口必须继续作为下一目标推进，不是停止点。
