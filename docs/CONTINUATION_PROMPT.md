...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐 A/E 数据源缺口闭环。loop816/SYNC-831 已把 missing-source onboarding surface/API contract 纳入 `product_self_validation_gate_v1`。下一步是 `MISSING_SOURCE_ONBOARDING_CANDIDATE_AVAILABILITY_EXPLANATION_LOOP817`：把缺源接入请求和 E 类候选 Data Availability Gate 解释对齐，让候选预览/复盘说明哪些 E 类候选因 `entity_linking_table` 或 `alternative_data_snapshot` 停在缺源待接入。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `MISSING_SOURCE_ONBOARDING_CANDIDATE_AVAILABILITY_EXPLANATION_LOOP817`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-831 已把缺失数据源接入请求纳入 product self-validation gate。下一步要让候选可用性解释消费这些缺源信息，缺口必须继续作为下一目标推进，不是停止点。
