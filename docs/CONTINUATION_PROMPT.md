...
- Business mainline: `apps/quant_assistant` — 因子构造宇宙真实可用链路正在补齐 A/E 数据源缺口闭环。loop827/SYNC-842 已把 `source_adapter_manifest_owner_approval_v1` 展示到 `/quant/factor-mining`；用户能看见 pending/approved owner approval 与 no-ready/no-execution 边界。下一步是 `DATA_AVAILABILITY_OWNER_APPROVAL_LINKAGE_LOOP828`：让 Data Availability Gate 消费 owner approval contract。
...
- **Next direction**: 读 `harness/loop-state.json`（当前业务下一原子动作是 `DATA_AVAILABILITY_OWNER_APPROVAL_LINKAGE_LOOP828`；Function-First Loop Gate v1.6 要求先声明 `core_function_artifact` / `phase_plan` / `functional_acceptance` / `closing_work_only`）
- Recovery: CodeX 有效性自检 → `harness/scripts/codex-self-check.ps1 -Format markdown`
- State facts: 见 `docs/PROJECT_STATUS.md` §5、`harness/loop-state.json` 与最新台账；SYNC-842 已建立 owner approval UI surface。下一步要把 owner approval contract 接入 Data Availability approved source-review state，缺口必须继续作为下一目标推进，不是停止点。
